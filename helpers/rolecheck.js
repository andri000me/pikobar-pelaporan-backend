...
const countByRole = (user, caseAuthors=[]) => {
  ...
  } else if (user.role == ROLE.PROV || user.role == ROLE.ADMIN) {
    searching = {}
  } else {
    if (user.unit_id && Array.isArray(caseAuthors) && caseAuthors.length) {
      searching = {
        $or: [
          ...
          { transfer_to_unit_id: new ObjectId(user.unit_id._id), transfer_status: VERIFIED_STATUS.APPROVED }
        ]
      }
    } else {
      searching = {
    ...
    if(user.role == ROLE.KOTAKAB){
      ...
      if (user.unit_id && Array.isArray(caseAuthors) && caseAuthors.length) {
        params.$or = [
          ...
          { transfer_to_unit_id: new ObjectId(user.unit_id._id), transfer_status: VERIFIED_STATUS.APPROVED, $or: search_params  }
        ]
